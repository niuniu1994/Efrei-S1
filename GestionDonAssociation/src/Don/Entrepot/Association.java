...
public class Association extends Destination {
    ...
    public static void rechercherDonParEtiquette(Etiqutte typeDeMobilier){
        File file = new File("src/RDPEtiquetteDansAsso.txt");
        try {
            if(!file.exists())
                file.createNewFile();
            BufferedWriter writer = new BufferedWriter(new FileWriter(file));
            for(int i = 0; i < association.getSize(); i++){
                if(association.getDon(i).getTypeMateriel().getEtiquette() == typeDeMobilier) {
                    System.out.println(association.getDon(i));
                    writer.write(association.getDon(i).toString() + "\r\n");
                }
            }
            writer.close();
